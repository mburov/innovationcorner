...
post '/compare' do
  ...
  if response.face_matches.count > 1
    ...
    {:message => "No match found!"}.to_json
  else
    ...
    {:id => response.face_matches[0].face.external_image_id, :confidence => response.face_matches[0].face.confidence, :message => "Hello "}.to_json
  end
end
